...
async def run_commands(sdk_conn):
    ...
    try:
        for x in commands_to_cozmo['commands']:
            ...
            if x['command'] == "TEST":
                ...
                set_cube_lights(cube_id, color_cube, color_cube_time, robot)
...
async def say(text_to_say, robot):
    print("COZMO TO SAY")
    global total 
    print (total)
    await robot.say_text(text_to_say, in_parallel=True).wait_for_completed()
...
def set_cube_lights(cube_id, color_cube, color_cube_time, robot):
...
def stack_cubes(cube_up, cube_down, robot):
    ...
    if len(cubes) < 2:
        ...
        if (cube_to_up and cube_to_down):
            # cube_to_up.set_lights(cozmo.lights.green_light.flash())
            # cube_to_down.set_lights(cozmo.lights.green_light.flash())
            ...
            # cube_to_up.set_light_corners(None, None, None, None)
            # cube_to_down.set_light_corners(None, None, None, None)
...
class SnippetList(mixins.ListModelMixin,
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            cozmo.connect(run_commands, connector=cozmo.run.FirstAvailableConnector())
